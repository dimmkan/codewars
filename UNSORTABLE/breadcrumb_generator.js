...
function deleteExtension(chunk) {
  return chunk.replace(/\.html/g, '').replace(/\.htm/g, '').replace(/\.php/g, '').replace(/\.asp/g, '');
}

function cutter(chunk) {
  if (chunk.length <= 30) {
    chunk = chunk.replace(/-/g, ' ');
  } else {
      const ignore = ["the","of","in","from","by","with","and", "or", "for", "to", "at", "a"];
      chunk = chunk.split('-').filter(e => !ignore.includes(e)).map(e => e[0]).join('');
  }

  return chunk.toUpperCase();
}

function buildSegment(url, chunk, last=false) {
  if (last) {
      return `<span class="active">${cutter(deleteExtension(chunk))}</span>`;
  } else {
      return `<a href="${url}">${cutter(deleteExtension(chunk))}</a>`;
  }
}
...
// Optimize
// function generateBC(url, separator) {
//   var words = ["the","of","in","from","by","with","and", "or", "for", "to", "at", "a"];
//   return url.replace(/(^.*?\w(?=\/|$))|(\/index\..*)|(\.(htm|php|asp).*)|([?#].*)|(\/$)/gi, '').split('/').reduce((result, link, i, a, label) => {
//     label = i ? link.replace(/-/g, ' ').replace(/([ \w]{31,})/g, x => x.replace(/(\w+) ?/g, (_, word) => words.includes(word) ? '' : word[0])).toUpperCase() : 'HOME';
//     result.link += link + '/';
//     result.html += i < a.length - 1 ? `<a href="${result.link}">${label}</a>${separator}` : `<span class="active">${label}</span>`;
//     return result;
//   }, {html: '', link: ''}).html
// }
