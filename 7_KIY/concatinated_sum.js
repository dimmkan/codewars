...
const getPositiveSum = (n, m) => n.toString().split('').map(v => +v.repeat(m)).reduce((acc, item) => acc + item, 0);
const getNegativeSum = (n, m) => n.toString().split('').map(v => +v.repeat(m)).reduce((acc, item) => acc - item, 0);


const checkConcatenatedSum = (n, m) => n > 0 ? getPositiveSum(n, m) === n : getNegativeSum(-n, m) === n;