...
const uglifyWord = (s) => s.split('').reduce((acc, item) => {
    if (acc.flag && item.match(/[a-z]/i)) {
        acc.flag = !acc.flag;
        acc.str += item.toUpperCase();
    } else if (!acc.flag && item.match(/[a-z]/i)) {
        acc.flag = !acc.flag;
        acc.str += item.toLowerCase();
    } else {
        acc.flag = true;
        acc.str += item;
    }
