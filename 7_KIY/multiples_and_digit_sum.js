...
const procedure = (n) => Array.from({ length: 100 }, (_, i) => ((n + i) % n === 0 && (n + i) <= 100) ? (n + i) : 0).filter(v => !!v).map(v => v.toString().split('').map(itm => ~~itm).reduce((acc, item) => acc + item)).reduce((acc, item) => acc + item);