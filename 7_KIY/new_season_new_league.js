...
const premierLeagueStandings = teamStandings => {
    const sortable = Object.values(teamStandings).sort((a, b) => {
        if (a > b) {
            return 1
        } else {
            return -1
        }
    }).filter(item => item != teamStandings[1]).reduce((acc, item, idx) => ({ ...acc, [idx + 2]: item }), {});
    return { 1: teamStandings[1], ...sortable };
}; 